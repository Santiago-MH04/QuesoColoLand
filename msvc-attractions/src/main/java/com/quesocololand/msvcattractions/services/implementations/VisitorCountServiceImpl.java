...
public class VisitorCountServiceImpl implements VisitorCountService {
    ...
    public List<GroupedVisitorCountDTO> getGroupedVisitorCounts(String attractionId, LocalDate date, int intervalMinutes) {
        // Validate the time-lapse is one of the permitted
        if (intervalMinutes != 5 && intervalMinutes != 15 && intervalMinutes != 30 && intervalMinutes != 60) {
            throw new IllegalArgumentException("Interval must be 5, 15, 30, or 60 minutes.");
        }
        ...
        SortOperation sortOperation = sort(Sort.by(Sort.Order.asc("_id.intervalStartx")));

        // 4. $project: formats the output, so it matches with GroupedVisitorCountDTO
        // _id es el campo que contiene nuestra clave de agrupación (intervalStart, attractionId)
        ProjectionOperation projectOperation = project()
            .and("$_id.attractionId").as("attraction_id")
            .and("$_id.intervalStart").as("interval_start")
